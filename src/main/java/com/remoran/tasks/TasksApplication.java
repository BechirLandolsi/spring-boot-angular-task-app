package com.remoran.tasks;

import com.remoran.tasks.domain.Task;
import com.remoran.tasks.service.TaskService;
...
public class TasksApplication {
    ...
    CommandLineRunner runner(TaskService taskService) {
        return args -> {
            ...
            taskService.save(new Task(5L, "Create the command line runner to load data", LocalDate.now().plus(6, ChronoUnit.DAYS), false));
            taskService.save(new Task(6L, "Create the required configuration properties", LocalDate.now().plus(8, ChronoUnit.DAYS), false));
